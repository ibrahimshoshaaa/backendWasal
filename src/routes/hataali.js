...
router.get('/my', async (req, res) => {
  try {
    ...
    console.error('GET /hataali/my error:', err);
...
router.get('/available', async (req, res) => {
  try {
    ...
    console.error('GET /hataali/available error:', err);
    res.status(500).json({ error: 'تعذر تحميل الطلبات المتاحة' });
  }
});
...
router.post('/:id/accept', async (req, res) => {
  try {
    ...
    console.error('POST /hataali/:id/accept error:', err);
...
router.get('/admin/all', async (req, res) => {
  try {
    ...
    console.error('GET /hataali/admin/all error:', err);
...
router.put('/admin/:id', async (req, res) => {
  try {
    ...
    if (status === 'approved') {
      ...
      // notifType مختلف عن orders ('new_hataali_order' مش 'new_available_order')
      // عشان الـ id بتاع الجدولين مستقل ومحتمل يتطابق بالصدفة.
      notifyOnlineDrivers(
        {
          title: 'طلب هاتهالي جديد! 🛵',
          body: `طلب "${order.title}" في انتظار مندوب يستلمه`,
          type: 'new_hataali_order',
          orderId: order.id,
        },
        sendToUser
      ).catch(() => {});
    ...
    console.error('PUT /hataali/admin/:id error:', err);
    res.status(500).json({ error: 'تعذر تحديث الطلب' });
  }
});

// GET /api/hataali/settings — رسوم التوصيل الافتراضية (أي مستخدم مسجّل دخول، مش الأدمن بس)
